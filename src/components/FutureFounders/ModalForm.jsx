  ...
  return (
    <>
      ...
      >
        ...
        <input type="text" name="phone" defaultValue={formik.values.phone} hidden />
        <input type="text" name="linkedinUrl" defaultValue={formik.values.linkedinUrl} hidden />
        <input type="text" name="twitterUrl" defaultValue={formik.values.twitterUrl} hidden />
        <input type="text" name="githubUrl" defaultValue={formik.values.githubUrl} hidden />
        <input type="text" name="portfolioUrl" defaultValue={formik.values.portfolioUrl} hidden />
        <input type="text" name="otherLink" defaultValue={formik.values.otherLink} hidden />
        <input type="text" name="file" defaultValue={formik.values.file} hidden />
        <textarea name="areasOfInterest" defaultValue={formik.values.areasOfInterest} hidden />
        <input type="text" name="referrals" defaultValue={formik.values.referrals} hidden />
        <input type="text" name="USEmploymentAuthorization" defaultValue={formik.values.USEmploymentAuthorization}
               hidden />
        <textarea name="anythingElse" defaultValue={formik.values.anythingElse} hidden />
        <input type="text" name="gender" defaultValue={formik.values.gender} hidden />
        <input type="text" name="race" defaultValue={formik.values.race} hidden />
        <input type="text" name="veteranStatus" defaultValue={formik.values.veteranStatus} hidden />
      </NetlifyForm>
      {isOpen && (
            ...
            {!isSent ? (
              <>
                ...
                >
                  ...
                  <div className={[
                    'field-wrapper',
                    formik.touched.linkedinUrl && Boolean(formik.errors.linkedinUrl) && 'error',
                  ].filter(Boolean).join(' ')
                  }>
                    ...
                    {formik.touched.linkedinUrl && Boolean(formik.errors.linkedinUrl) && (
                      <span className="helper-text">{formik.errors.linkedinUrl}</span>
                    )}
                  </div>
                  ...
                  <div className="comment">Feel free to submit anything else you feel would be
                    helpful or would like us to consider – code, website, slide deck, video, etc.
                  </div>
                  <h3>Referrals</h3>
                  <div className="comment">Know someone who would be a good fit for the Future
                    Founders program? Please provide their LinkedIn or email.
                  </div>
                  <div className="field-wrapper">
                  ...
                  <h3>Anything else?</h3>
                  <div className="comment">You can add a note, cover letter, areas of interest,
                    unique background in a specific vertical, or anything else you might want to
                    share.
                  </div>
                  <textarea
                    id="anythingElse"
                  ...
                  <div className="comment">Do you now, or will you in the future, require
                    sponsorship for employment visa status to work legally for our Company in the
                    US? *
                  </div>
                  <div className={[
                    'field-wrapper', 'field-radio-wrapper',
                    formik.touched.USEmploymentAuthorization &&
                    Boolean(formik.errors.USEmploymentAuthorization) && 'error'
                  ].filter(Boolean).join(' ')
                  }>
                    <label className='label-container'>
                      <input
                        ...
                        value="Yes"
                        onChange={formik.handleChange}
                        checked={formik.values.USEmploymentAuthorization === "Yes"}
                      />
                      Yes
                    </label>
                    <label className='label-container'>
                    ...
                    Boolean(formik.errors.USEmploymentAuthorization) && (
                      <span className="helper-text">{formik.errors.USEmploymentAuthorization}</span>
                    )}
                  </div>
                  <h3>Equal Employment Opportunity Information</h3>
                  <div className="comment">Completion is voluntary and will not subject you to
                    adverse treatment.<br /><br />Our company values diversity. To ensure that we
                    comply with reporting requirements and to learn more about how we can increase
                    diversity in our candidate pool, we invite you to voluntarily provide
                    demographic information in a confidential survey at the end of this application.
                    Providing this information is optional. It will not be accessible or used in the
                    hiring process, and has no effect on your opportunity for employment.
                  </div>
                  <div className="field-wrapper">
                  ...
                  <div className="field-wrapper">
                    ...
                    <Select
                      ...
                      theme={themeSelect}
                      onChange={(option) => formik.setFieldValue("race", option.value)}
                    />
                  </div>
                  <div className="field-wrapper">
                    ...
                    <Select
                      ...
                      theme={themeSelect}
                      onChange={(option) => formik.setFieldValue("veteranStatus", option.value)}
                    />
                  </div>
                  <Recaptcha
